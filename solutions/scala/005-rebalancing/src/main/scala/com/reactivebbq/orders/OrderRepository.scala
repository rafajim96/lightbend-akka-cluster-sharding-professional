...
class SQLOrderRepository()(implicit ec: ExecutionContext) extends OrderRepository {
  ...
  private def transaction(f: EntityManager => OrderDBO): OrderDBO = {
    ...
    if(result != null)
      entityManager.refresh(result) // Bypass results from the L1 cache.
  ...
  override def find(orderId: OrderId): Future[Option[Order]] = Future {
    val dbo = transaction(_.find(classOf[OrderDBO], orderId.value))

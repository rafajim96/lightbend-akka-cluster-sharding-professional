...
object OrderActor {
  ...
  case class OrderLoaded(order: Option[Order])
...
class OrderActor(repository: OrderRepository) extends Actor
  ...
  private def loading: Receive = {
    ...
    case Status.Failure(ex) =>
      log.error(ex, message = s"[$orderId] FAILURE: ${ex.getMessage}")
      throw ex // default supervision restarts actor on exception
    case _ => stash()
  }
  ...
  private def addItem(order: Order, orderItem: OrderItem): Future[ItemAddedToOrder] = {
    repository.update(order.withItem(orderItem))
